...
async function connectToDatabase() {
    try {
        db = await mysql.createConnection({
            host: 'your_host', // e.g., 'localhost' or 'your_ip_address'
            user: 'your_user', // e.g., 'db_username'
            password: 'your_password', // e.g., 'db_password'
            database: 'your_database', // e.g., 'database_name'
...
app.get('your_endpoint', async (req, res) => {
    try {
        const response = await axios.get('http://your_ip');
...
app.get('your_endpoint', async (req, res) => {
    try {
        ...
        const response = await axios.get('https://your_ip', {
            headers: {
                'Authorization': `Basic ${Buffer.from('your_api_user:your_api_password').toString('base64')}`
            },
