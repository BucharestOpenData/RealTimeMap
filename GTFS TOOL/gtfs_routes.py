...
debug_json_file_path = os.path.join(download_dir, "debug_data.json")
        ...
        for name, group in tqdm(grouped, desc=Fore.WHITE + "Generating LineStrings" + Style.RESET_ALL):
            ...
            if len(unique_points) > 1:
                ...
                if line.is_valid:  # Ensure the geometry is valid
                    ...
                    route_names.append(group['route_short_name'].iloc[
                                           0])  # Assuming all rows in the group have the same route_short_name
...
# Schedule the tasks
schedule.every().day.at("03:00").do(fetch_and_extract_gtfs)
schedule.every().day.at("03:30").do(generate_geojson_routes)

# Run the tasks immediately after scheduling
fetch_and_extract_gtfs()
generate_geojson_routes()


# Flask route for debugging
@app.route("/debug", methods=["GET"])
def debug():
    if os.path.exists(debug_json_file_path):
        with open(debug_json_file_path) as f:
            return jsonify(json.load(f))
    return jsonify({"message": "No debug data found."})


if __name__ == "__main__":
    # Use tqdm to run the schedule
    while True:
